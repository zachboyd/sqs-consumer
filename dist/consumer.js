...
function toSQSError(err, message) {
    ...
    sqsError.retryable = (_a = err.$retryable) === null || _a === void 0 ? void 0 : _a.throttling;
    sqsError.hostname = err.$service;
...
class Consumer extends events_1.EventEmitter {
    ...
    async deleteMessageBatch(messages) {
        ...
        try {
            await this.sqs.deleteMessageBatch(deleteParams);
        }
        catch (err) {
            throw toSQSError(err, `SQS delete message failed: ${err.message}`);
        }
    ...
    async changeVisabilityTimeoutBatch(messages, timeout) {
        ...
        try {
            return this.sqs.changeMessageVisibilityBatch(params);
        }
        catch (err) {
            this.emit('error', err, messages);
        }
